...
export default function TestNewLogoPage() {
  ...
  const checkAllLogos = async () => {
    ...
    for (const favicon of faviconFiles) {
      try {
        const response = await fetch(`${favicon.url}${currentCacheBuster}`, {
          method: 'HEAD',
          cache: 'no-cache'
        });
    ...
    <div className="min-h-screen bg-gray-100 py-8">
      <div className="max-w-6xl mx-auto px-4">
        <div className="bg-white rounded-lg shadow-lg p-8">
          <h1 className="text-3xl font-bold mb-6 text-center">🎨 Test New Logo</h1>
          <p className="text-gray-600 mb-8 text-center">
            ตรวจสอบโลโก้ใหม่ "Smart Shrimp Farm Management App" ในทุกขนาด
          </p>
          
          {/* PWA Icons */}
          <div className="mb-8">
            <h2 className="text-xl font-semibold mb-4 text-center">PWA Icons</h2>
          ...
          {/* Favicons */}
          <div className="mb-8">
            <h2 className="text-xl font-semibold mb-4 text-center">Favicons</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {faviconFiles.map((favicon) => (
                <div key={favicon.name} className="text-center p-4 border-2 border-gray-200 rounded-lg">
                  ...
                  <div className="relative">
                    <img
                      src={`${favicon.url}${isClient ? cacheBuster : ''}`}
                      alt={favicon.name}
                      className="w-16 h-16 mx-auto rounded"
                      onLoad={() => setLogoStatus(prev => ({ ...prev, [favicon.name]: true }))}
                      onError={() => setLogoStatus(prev => ({ ...prev, [favicon.name]: false }))}
                    />
                    {logoStatus[favicon.name] === true && (
                      <div className="absolute top-1 right-1 bg-green-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-xs">
                        ✓
                      </div>
                    )}
                    {logoStatus[favicon.name] === false && (
                      <div className="absolute top-1 right-1 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-xs">
                        ✗
                      </div>
                    )}
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
            <h3 className="font-semibold text-blue-800 mb-2">วิธีตรวจสอบโลโก้ใหม่:</h3>
            <ol className="text-blue-700 text-sm space-y-1 list-decimal list-inside">
              <li>ดูที่ tab ของเบราว์เซอร์ (ควรเป็นโลโก้ใหม่ "Smart Shrimp Farm")</li>
              <li>ดูที่ bookmark (ถ้ามี)</li>
              <li>ดูที่ PWA icon (ถ้าติดตั้งแล้ว)</li>
              <li>ดูที่ mobile home screen (ถ้าติดตั้ง PWA)</li>
              <li>ดูที่ notification (ถ้าทดสอบ push notification)</li>
            </ol>
          </div>
